...
func (s *voucherRepository) Create(ctx context.Context, voucher interface{}) (string, error) {
	...
	u, ok := voucher.(models.CreateVoucherReq)
	if !ok {
		tx.Rollback()
		return "", errors.New("el tipo de voucher no es correcto")
	}

	var existingVoucher models.CreateVoucherReq

	if err := tx.Where("voucher = ?", u.Voucher).First(&existingVoucher).Error; err == nil {
...
func (s *voucherRepository) GetInf(ctx context.Context, filter map[string]interface{}, skip, take *int) ([]interface{}, error) {
	return nil, nil
}

func (s *voucherRepository) Get(ctx context.Context, filter map[string]interface{}, skip, take *int) ([]interface{}, error) {

	var registro []models.VoucherResp
	...
	countQuery := DB.WithContext(ctx).Model(&models.VoucherResp{})
	if len(filter) > 0 {
		for key, value := range filter {
	...
	query := nDB.WithContext(ctx).Model(&models.VoucherResp{})
	...
	response := models.VoucherListResponse{
		Items:      registro,
...
func (s *voucherRepository) GetByID(ctx context.Context, ID string) (interface{}, error) {
	...
	if err := DB.WithContext(ctx).Where("id = ?", ID).First(&voucher).Error; err != nil {
...
func (s *voucherRepository) Update(ctx context.Context, ID string, voucher interface{}) error {
	...
	if err := DB.WithContext(ctx).Model(&models.UpdateVoucherReq{}).Where("id = ?", ID).Updates(voucher).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			DB.Rollback()
...
func (s *voucherRepository) Delete(ctx context.Context, ID string, filter map[string]interface{}) error {
	//asigna el schma a usar
	DB := infrastructure.GetDBWithSchema(ctx, s.DB).Begin()
	...
	result := DB.WithContext(ctx).Delete(&models.Vouchers{}, "id = ?", ID)
	...
	if result.RowsAffected == 0 {
		DB.Rollback()
		return errors.New("voucher no encontrado ") // Manejo de error si no se encontró el registro
	}
