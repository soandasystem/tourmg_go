...
type ProgramacRepository struct {
	infrastructure.PostgresRepositoryOrm
}

// NewUserRepository creates a roles repository for postgres
func NewProgramacRepository(ctx context.Context, db *gorm.DB) ports.ProgramacRepository {
	return &ProgramacRepository{
		infrastructure.PostgresRepositoryOrm{
			DB: db,
		},
	}
...
func (s *ProgramacRepository) Create(ctx context.Context, program interface{}) (string, error) {
	//asigna el schma a usar
	DB := infrastructure.GetDBWithSchema(ctx, s.DB)
	// Asegúrate de que el tipo del usuario es correcto
	u := program.(models.CreateProgramacReq)

	var existingReg models.CreateProgramacReq
...
func (s *ProgramacRepository) GetInf(ctx context.Context, filter map[string]interface{}, skip, take *int) ([]interface{}, error) {
	return nil, nil
}

func (s *ProgramacRepository) Get(ctx context.Context, filter map[string]interface{}, skip, take *int) ([]interface{}, error) {
	//asigna el schma a usar
	DB := infrastructure.GetDBWithSchema(ctx, s.DB)

	var registro []models.ProgramacResp

	// Crea una consulta base
	query := DB.WithContext(ctx).Model(&models.ProgramacResp{})
	...
	response := models.ProgramacListResponse{
		Items:      registro,
		TotalCount: 0,
	}
...
func (s *ProgramacRepository) GetByID(ctx context.Context, ID string) (interface{}, error) {
	//asigna el schma a usar
	DB := infrastructure.GetDBWithSchema(ctx, s.DB)
...
func (s *ProgramacRepository) Update(ctx context.Context, ID string, program interface{}) error {
	//asigna el schma a usar
	DB := infrastructure.GetDBWithSchema(ctx, s.DB)

	if err := DB.WithContext(ctx).Model(&models.UpdateProgramacReq{}).Where("id = ?", ID).Updates(program).Error; err != nil {
...
func (s *ProgramacRepository) Delete(ctx context.Context, ID string, filter map[string]interface{}) error {
	//asigna el schma a usar
	DB := infrastructure.GetDBWithSchema(ctx, s.DB)
