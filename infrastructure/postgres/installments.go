...
type installmentsRepository struct {
	infrastructure.PostgresRepositoryOrm
}

// NewUserRepository creates a roles repository for postgres
func NewInstallmentsRepository(ctx context.Context, db *gorm.DB) ports.InstallmentsRepository {
...
func (s *installmentsRepository) Create(ctx context.Context, installments interface{}) (string, error) {
	...
	u, ok := installments.(models.CreateInstallmentReq)
	if !ok {
		tx.Rollback()
		return "", errors.New("el tipo de installments no es correcto")
	}
...
func (s *installmentsRepository) Get(ctx context.Context, filter map[string]interface{}, skip, take *int) ([]interface{}, error) {
	var registro []models.InstallmentResp
	...
	query := DB.WithContext(ctx).Model(&models.InstallmentResp{})
	...
	// Ejecuta la consulta
	if err := query.Preload("Passenger").Preload("Sale").Preload("Sale.Establecimiento").Order("id ASC").Find(&registro).Error; err != nil {
		return nil, err
	}
...
func (s *installmentsRepository) GetInf(ctx context.Context, filter map[string]interface{}, skip, take *int) ([]interface{}, error) {
	var registro []models.InstallmentInf
	var totalCount int64
	...
	// Crea una consulta base
	query := DB.WithContext(ctx).Model(&models.InstallmentInf{})
	...
	// Mapear a la estructura de respuesta
	response := models.InstallmentInfListResponse{
		Items:      registro,
		TotalCount: totalCount,
	}
	// Devolver como slice de interface{} para cumplir con la interfaz
...
func (s *installmentsRepository) Update(ctx context.Context, ID string, installments interface{}) error {
	...
	if err := DB.WithContext(ctx).Model(&models.UpdateInstallmentReq{}).Where("id = ?", ID).Updates(installments).Error; err != nil {
