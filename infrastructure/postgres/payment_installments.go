...
// paymentInstallmentsRepository adapter of an paymentInstallments repository for postgres
type paymentInstallmentsRepository struct {
	infrastructure.PostgresRepositoryOrm
}

// NewPaymentInstallmentsRepository creates a paymentInstallments repository for postgres
func NewPaymentInstallmentsRepository(ctx context.Context, db *gorm.DB) ports.PaymentInstallmentRepository {
	return &paymentInstallmentsRepository{
		infrastructure.PostgresRepositoryOrm{
			DB: db,
		},
	}
...
func (s *paymentInstallmentsRepository) Create(ctx context.Context, pagos interface{}) (string, error) {
	...
	paymentInstallment, ok := pagos.(models.CreatePaymentInstallmentReq)
	if !ok {
		tx.Rollback()
		return "", errors.New("el tipo de pagos no es correcto")
	}
	/*
		var existingPaymentInstallment models.CreatePaymentInstallmentReq


		if err := tx.Where("id = ?", paymentInstallment.ID).First(&existingPaymentInstallment).Error; err == nil {
			// Si no hay error, significa que se encontró un rol con ese nombre
			tx.Rollback()
			return "error", errors.New("El Ingreso con el identificador '" + paymentInstallment.ID + "' ya existe")
	...
	*/
	...
	return paymentInstallment.ID, nil
}

func (s *paymentInstallmentsRepository) Get(ctx context.Context, filter map[string]interface{}, skip, take *int) ([]interface{}, error) {
	var registro []models.PaymentInstallmentResp
	...
	query := DB.WithContext(ctx).Model(&models.PaymentInstallmentResp{})
	...
	response := models.PaymentInstallmentListResponse{
		Items:      registro,
...
func (s *paymentInstallmentsRepository) GetInf(ctx context.Context, filter map[string]interface{}, skip, take *int) ([]interface{}, error) {
	var registro []models.PaymentInstallmentInf
	var totalCount int64
	...
	// Crea una consulta base
	query := DB.WithContext(ctx).Model(&models.PaymentInstallmentInf{})
	...
	// Mapear a la estructura de respuesta
	response := models.PaymentInstallmentInfListResponse{
		Items:      registro,
		TotalCount: totalCount,
	}
	// Devolver como slice de interface{} para cumplir con la interfaz
...
func (s *paymentInstallmentsRepository) GetByID(ctx context.Context, ID string) (interface{}, error) {
	//asigna el schma a usar
	DB := infrastructure.GetDBWithSchema(ctx, s.DB)

	var paymentInstallment models.PaymentInstallmentResp

	// Busca el registro en la base de datos utilizando GORM
	if err := DB.WithContext(ctx).Where("id = ?", ID).First(&paymentInstallment).Error; err != nil {
...
func (s *paymentInstallmentsRepository) Update(ctx context.Context, ID string, paymentInstallment interface{}) error {
	//asigna el schma a usar
	...
	if err := DB.WithContext(ctx).Model(&models.UpdatePagosReq{}).Where("id = ?", ID).Updates(paymentInstallment).Error; err != nil {
...
func (s *paymentInstallmentsRepository) Delete(ctx context.Context, ID string, filter map[string]interface{}) error {
	//asigna el schma a usar
	DB := infrastructure.GetDBWithSchema(ctx, s.DB).Begin()
