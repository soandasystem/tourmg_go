...
func (s *usersRepository) Create(ctx context.Context, users interface{}) (string, error) {
	...
	// Asegúrate de que el tipo del registro es correcto
	u, ok := users.(models.CreateUsersReq)
	if !ok {
		tx.Rollback()
		return "", errors.New("el tipo de usuario no es correcto")
	}
	var existingReg models.CreateUsersReq

	if err := tx.Where("username = ?", u.Username).First(&existingReg).Error; err == nil {
...
func (s *usersRepository) GetInf(ctx context.Context, filter map[string]interface{}, skip, take *int) ([]interface{}, error) {
	return nil, nil
}

func (s *usersRepository) Get(ctx context.Context, filter map[string]interface{}, skip, take *int) ([]interface{}, error) {
	var registro []models.UsersInf
	...
	query := DB.WithContext(ctx).Model(&models.UsersInf{})
	...
	// Ejecuta la consulta
	if err := query.Preload("Rol").Order("id").Table("users").Find(&registro).Error; err != nil {
		return nil, err
	}
...
func (s *usersRepository) GetByID(ctx context.Context, ID string) (interface{}, error) {
	...
	if err := DB.WithContext(ctx).Preload("Rol").Table("users").Where("id = ?", ID).First(&registro).Error; err != nil {
...
func (s *usersRepository) GetByIDUpdate(ctx context.Context, ID string) (interface{}, error) {
	//asigna el schma a usar
...
func (s *usersRepository) Update(ctx context.Context, ID string, users interface{}) error {
	...
	if err := DB.WithContext(ctx).Model(&models.UpdateUsersReq{}).Where("id = ?", ID).Updates(users).Error; err != nil {
...
func (s *usersRepository) Delete(ctx context.Context, ID string, filter map[string]interface{}) error {
	...
	result := DB.WithContext(ctx).Delete(&models.Users{}, "id = ?", ID)
	...
	if result.RowsAffected == 0 {
		DB.Rollback()
		return errors.New("usuario no encontrado ") // Manejo de error si no se encontró el registro
	}
