...
func SetProgramacRoutes(ctx context.Context, cfg config.Config, r *gin.Engine, p ports.ProgramacService) {
	//	r.Use(middlewares.Recover())

	r.POST("/api/v3.5/programac", createProgramac(p))
	r.GET("/api/v3.5/programac", getAllProgramac(p))
	r.GET("/api/v3.5/programac/:id", getProgramacByID(p))
	r.PATCH("/api/v3.5/programac/:id", updateProgramac(p))
	r.DELETE("/api/v3.5/programac/:id", deleteProgramac(p))
}
...
func updateProgramac(p ports.ProgramacService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		params := c.Params
		var program models.UpdateProgramacReq
		err = json.Unmarshal(body, &program)
		if err != nil {
			response := util.NewErrorResponse(err, http.StatusInternalServerError)
			c.JSON(response.StatusCode, response)
