...
func SetProgramsRoutes(ctx context.Context, cfg config.Config, r *gin.Engine, p ports.ProgramsService) {
	//	r.Use(middlewares.Recover())

	r.POST("/api/v3.5/programs", createPrograms(p))
	r.GET("/api/v3.5/programs", getAllPrograms(p))
	r.GET("/api/v3.5/programs/:id", getProgramsByID(p))
	r.PATCH("/api/v3.5/programs/:id", updatePrograms(p))
	r.DELETE("/api/v3.5/programs/:id", deletePrograms(p))
}

// @Summary Create program
// @Description Creates a new program
// @Tags program
// @Param user body models.CreateProgramsReq true "New program to be created"
// @Success 201 {object} models.ProgramsResp "OK"
...
func createPrograms(p ports.ProgramsService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var program models.CreateProgramsReq
...
func updatePrograms(p ports.ProgramsService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		params := c.Params
		var program models.UpdateProgramsReq
		err = json.Unmarshal(body, &program)
		if err != nil {
			response := util.NewErrorResponse(err, http.StatusInternalServerError)
			c.JSON(response.StatusCode, response)
