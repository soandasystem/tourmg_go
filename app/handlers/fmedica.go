...
func SetFmedicaRoutes(ctx context.Context, cfg config.Config, r *gin.Engine, p ports.FmedicaService) {
	...
	r.POST("/api/v3.5/ficha", createFicha(p))
	r.GET("/api/v3.5/ficha", getAllFicha(p))
	r.GET("/api/v3.5/ficha/:id", getFichaByID(p))
	r.PATCH("/api/v3.5/ficha/:id", updateFicha(p))
	r.DELETE("/api/v3.5/ficha/:id", deleteFicha(p))
}
